...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in array head; do
        steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj ${nj} --write_utt2num_frames true \
            data/${x} exp/make_fbank/${x} ${fbankdir}
        utils/fix_data_dir.sh data/${x}
    done

    utils/combine_data.sh --extra_files utt2num_frames data/${train_set_array}_org data/array
    utils/combine_data.sh --extra_files utt2num_frames data/${dev_set_array}_org data/array
    utils/combine_data.sh --extra_files utt2num_frames data/${train_set_head}_org data/head
    utils/combine_data.sh --extra_files utt2num_frames data/${dev_set_head}_org data/head
    ...
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
        data/array/feats.scp data/train_set_array/cmvn.ark exp/dump_feats/recog/test_set_array \
        ${feat_recog_dir_array}

    feat_recog_dir_head=${dumpdir}/test_set_head/delta${do_delta}
    mkdir -p feat_recog_dir_head
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
        data/head/feats.scp data/train_set_head/cmvn.ark exp/dump_feats/recog/test_set_head \
        ${feat_recog_dir_head}
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    data2json.sh --feat ${feat_recog_dir}/feats.scp --bpecode ${bpemodel}.model \
        data/array ${dict} >${feat_recog_dir}/data_${bpemode}${nbpe}.json
    echo  'End Stage V'

    echo 'Start Stage VI'
    feat_recog_dir=${dumpdir}/test_set_head/delta${do_delta}
    data2json.sh --feat ${feat_recog_dir}/feats.scp --bpecode ${bpemodel}.model \
        data/head ${dict} >${feat_recog_dir}/data_${bpemode}${nbpe}.json
    echo 'End Stage VI'
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    if [[ $(get_yaml.py ${train_config} model-module) = *transformer* ]] ||
        ...
        # --num 5
        # average_checkpoints_multi.py --log exp/trainset_pytorch_train_specaug/results/log --backend pytorch --snapshots exp/trainset_pytorch_train_specaug/results/snapshot.ep.* --out exp/trainset_pytorch_train_specaug/results/model.acc.bests --num 5
    ...
    for rtask in ${recog_set}; do
        (
            ...
            # split data
            splitjson_multi.py --parts ${nj} ${feat_recog_dir}/data_${bpemode}${nbpe}.json
            # splitjson_multi.py --parts 12 dump/test_clean/deltafalse/data_unigram5000.json
