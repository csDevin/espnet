...
from espnet.transform.transformation_multi import Transformation
...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
        ...
        """
        # batch: List of tuple
        # uttid_list ['F04-Session1-arrayMic-0006', 'F04-Session1-arrayMic-0007', 'F04-Session1-arrayMic-0005'] List of utterence keys
        # info: {'input': [{...}], 'output': [{...}], 'utt2spk': 'F04'}
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info["input"]):
                    ...
                    # {"input":
                    #  [{"array_feat": "some/path.h5:F01_050C0101_PED_REAL",
                    #    "head_feat":  "some/path.h5:F01_050C0101_PED_REAL",
                    #    "filetype": "hdf5",
                    #    "name": "input1", ...}], ...}

                    x_array = self._get_from_loader(
                        filepath=inp["array_feat"], filetype=inp.get("filetype", "mat")
                    )
                    x_head = self._get_from_loader(
                        filepath=inp["head_feat"], filetype=inp.get("filetype", "mat")
                    )
                    
                    x_feats_dict.setdefault(inp["name"], []).append((x_array,x_head))
        ...
        if self.mode == "asr":
            ...
            # new_return_batch = OrderedDict()
            # new_return_batch[list(return_batch.keys()[0]]=[]
            # for array_feat, head_feat in return_batch[list(return_batch.keys())[0]]:
            #     new_return_batch[list(return_batch.keys()[0]].append(array_feat)
        # elif self.mode == "tts":
        #     _, info = batch[0]
