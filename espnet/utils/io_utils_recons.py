...
from espnet.transform.transformation_recons import Transformation
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info["input"]):
                    ...
                    dys_feat = self._get_from_loader(
                        filepath=inp["dys_feat"], filetype=inp.get("filetype", "mat")
                    )
                    hc_feat = self._get_from_loader(
                        filepath=inp["hc_feat"], filetype=inp.get("filetype", "mat")
                    )
                    
                    x_feats_dict.setdefault(inp["name"], []).append((dys_feat,hc_feat))
