...
def batchfy_by_seq(
    ...
    while True:
        ...
        ilen = max(int(info[ikey][iaxis]["dys_shape"][0]),
                   int(info[ikey][iaxis]["hc_shape"][0]))
        olen = (
...
def batchfy_by_bin(
    ...
    idim = max(int(sorted_data[0][1][ikey][0]["dys_shape"][1]),
               int(sorted_data[0][1][ikey][0]["hc_shape"][1]))
...
def make_batchset(
    ...
    for d in category2data.values():
        ...
        sorted_data = sorted(
            d.items(),
            key=lambda data: max(int(data[1][batch_sort_key][batch_sort_axis]["dys_shape"][0]), int(
                data[1][batch_sort_key][batch_sort_axis]["hc_shape"][0])),
