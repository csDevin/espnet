            ...
            for idx in range(len(self.conf["process"])):
                ...
                try:
                    if uttid_list is not None and "uttid" in param:
                        xs = [(func(x1, u, **_kwargs),func(x2, u, **_kwargs)) for x1,x2, u in zip(xs, uttid_list)]
                    else:
                        xs = [(func(x1, **_kwargs),func(x2, **_kwargs)) for x1,x2 in xs]
                except Exception:
                    logging.fatal(
...
# test case
        # if load_tr.preprocessing.conf.get("mode", "sequential") == "sequential":
        #     for idx in range(len(load_tr.preprocessing.conf["process"])):
        #         func = load_tr.preprocessing.functions[idx]
        #         # TODO(karita): use TrainingTrans and UttTrans to check __call__ args
        #         # Derive only the args which the func has
        #         try:
        #             param = signature(func).parameters
        ...
        #             if uttid_list is not None and "uttid" in param:
        #                 # False case
        #                 return_batch['input1'] = [func(x, u, **_kwargs) for x, u in zip(return_batch['input1'], uttid_list)]
        #             else:
        #                 return_batch['input1'] = [func(x, **_kwargs) for x in return_batch['input1']]
        #         except Exception:
        #             logging.fatal(
        #                 "Catch a exception from {}th func: {}".format(idx, func)
        #             )
